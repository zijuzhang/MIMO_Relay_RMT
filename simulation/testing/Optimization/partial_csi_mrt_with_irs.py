...
for snr in SNRs:
    ...
    for simulation_ind in range(num_simulations):
        ...
        beamformer_mrt = maximum_ratio_transmission((h2.T@np.diag(phase)@H1+los_channel).T)
        ...
        for i in range(per_channel_symbols_num):
                ...
                received_signal = (h2.T@np.eye(num_surfaces)@H1+los_channel)@(symbol*beamformer_mrt/np.sqrt(num_tx)) + noise
                received_signal_opt = (h2.T@np.diag(phase)@H1+los_channel)@(symbol*beamformer_opt/np.sqrt(num_tx)) + noise
...
plt.title(title)
plt.grid(True)
plt.plot(SNRs_dB, BER_list, label=f"No IRS Opt: # Pilots: {num_pilots_no_IRS}", linestyle='--', marker='o')
plt.plot(SNRs_dB, BER_list_opt,label=f"IRS Opt: # Pilots: {num_pilots_IRS}", linestyle='--', marker='x')
plt.legend(loc='lower left')
plt.show()
time_path = "Output/IRS_LOS_BER.png"
