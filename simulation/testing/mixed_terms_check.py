...
main_AED, bins_AED = np.histogram(np.linalg.eig(main)[0], bins=bins)
...
for i in range(average):
    ...
    main_AED += np.histogram(np.linalg.eig(main)[0], bins=bins)[0]
...
AED = main_AED/(cols*average)
ax.plot(bins_total[1::], AED, label='$GG^H+ HH^H$')
