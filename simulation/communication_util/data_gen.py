...
class CommunicationDataGenerator:
    ...
    def send_through_channel(self, quantization_level=None, plot=False, noise_levels=None):
        ...
        if noise_levels is not None:
            ...
            # check = np.random.random_integers(0, noise_levels, self.channel_output.shape)
    ...
    def reduced_state_mapping(self, num_clusters):
        ...
        # plt.scatter(np.real(x_list), np.imag(x_list), label="Received Signal")
        # plt.scatter(np.real(centroids), np.imag(centroids), label="Cluster Center")
        # plt.xlabel("Real")
        # plt.ylabel("Img")
