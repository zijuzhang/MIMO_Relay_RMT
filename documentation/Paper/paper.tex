...
RIS has no impact on the capacity of the system. Second, that for matrices of random elements, the  convergence of the matrix behavior towards that predicted by Free Probability is slowed by adding correlations between elements. Aligning these results with previous results showing the benefits of exploiting RIS phases in Section \ref{Background} requires further defining the parameters of systems which can be considered asymptotic. 
In order to at least numerically investigate this question we now compare the performance of RIS MIMO systems with parameters reflecting the state of the art. 
